...
- 여행·등록 장소·확정 일정은 SQLite `data/chiwawa.db`에 저장되어 서버를
  재시작해도 유지됩니다. `APP_DB_PATH`로 저장 위치를 명시적으로 변경할 수
  있습니다.
- 미확정 경로 계산 결과, 추천 조회 결과, OAuth state는 일시 상태이므로 서버
  재시작 시 초기화됩니다. 확정 일정은 다시 계산하지 않아도 복원됩니다.
