...
- 운영 여행 상태, OAuth state, 사용자와 Memorial 사진 메타데이터는 로컬 SQLite에
  저장되고 단일 호스트의 여러 worker가 공유한다.
- Memorial 원본은 모바일 앱의 로컬 저장소가 소유한다. 백엔드는 원본을 보관하지
  않고 여행별 사진 메타데이터만 저장한다.
- 운영의 모든 여행·회원 Memorial API는 JWT와 사용자 소유권으로 보호한다.
- 개발 여행 API만 로컬 호환을 위해 process-local 상태와 actor `0`을 허용한다.
- `/health`는 liveness, `/ready`는 설정·DB readiness다.
